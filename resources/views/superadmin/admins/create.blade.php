<x-layout>
    ...
    <div class="mx-auto max-w-3xl overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <form action="{{ route('superadmin.admins.store') }}" method="POST" class="space-y-5 p-6">@csrf
            ...
            <div class="flex justify-end gap-3 border-t border-slate-100 pt-5"><a href="{{ route('superadmin.admins.index') }}" class="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold">Batal</a><button class="rounded-md bg-yellow-300 px-4 py-2 text-sm font-semibold text-white hover:bg-yellow-400">Buat Akun</button></div>
        </form>
    </div>
</x-layout>
