...
class SuperAdminController extends Controller
{
    public function index()
    {
        $admins = User::where('role', 'admin')
            ->with(['group.kategori', 'group.layanans', 'group.prodis'])
            ->when(request('search'), fn ($query, $search) => $query->where(fn ($nested) => $nested->where('username', 'like', "%{$search}%")->orWhere('email', 'like', "%{$search}%")->orWhereHas('group', fn ($group) => $group->where('nama_group', 'like', "%{$search}%"))))
            ->when(request()->filled('status'), fn ($query) => $query->where('status', request('status')))
            ->when(request()->filled('kategori'), fn ($query) => $query->whereHas('group', fn ($group) => $group->where('kategori_id', request('kategori'))))
            ->get();

        return view('superadmin.admins.index', compact('admins'), [
            'title' => 'Super Admin',
            'kategoriList' => Kategori::orderBy('urutan')->get(),
        ]);
    }
